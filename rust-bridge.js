        ...
        try {
            ...
            const rustResult = {
                ...
                steps: activeSteps.map(step => ({
                    ...
                    note: (window.processSteps && step.agitation) ? window.processSteps.describeAgitation(step.agitation) : null
                }))
            };
...
function reportLines(results, steps = []) {
    ...
    lines.push('', 'Generated by DarkroomPro v1.1.0', 'Professional Film Development Calculator');
...
// Prevent display/idle sleep while the timer runs
let wakeLockSentinel = null;

async function setSleepBlock(enabled) {
    if (isDesktop && invoke) {
        try {
            await invoke('set_sleep_block', { enabled });
            return true;
        } catch (error) {
            console.warn('Sleep block failed:', error);
            return false;
        }
    }
    ...
    if (typeof navigator !== 'undefined' && 'wakeLock' in navigator) {
        try {
            if (enabled && !wakeLockSentinel) {
                wakeLockSentinel = await navigator.wakeLock.request('screen');
                wakeLockSentinel.addEventListener('release', () => { wakeLockSentinel = null; });
            } else if (!enabled && wakeLockSentinel) {
...
// Toggle OS fullscreen (darkroom focus mode)
